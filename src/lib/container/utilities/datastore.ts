...
/** Queue functions */
export function setItemToAutoSave(item: ItemType<Template>): void {
...
export function deleteItemFromAutoSave(item: ItemDeclaration<Template>): void {
...
function cleanItemQueue(bucketKey: string, itemKey: string): void {
...
function queueCall(fn: Callback, bucketKey: string, itemKey: string): ReturnType<typeof fn> {
...
export function dataStoreQueueCall<T extends Template = Template>(
	itemKey: string,
	...
	const [succes, message] = pcall((): void => {
		const callback = (
			data: T | undefined,
			keyInfo: DataStoreKeyInfo,
		): LuaTuple<[newValue: Template, userIds?: Array<number> | undefined, metadata?: object | undefined]> => {
			...
			if (corrupted === true) {
				...
				return [data as T, data!.userIds] as unknown as LuaTuple<
...
export function saveAsync(item: ItemDeclaration<Template>): void {
	const [{ data, metadata }] = dataStoreQueueCall(item.key, item.bucketOn, {
	...
	task.spawn(() => {
		game.BindToClose(() => {
			if (!RunService.IsStudio() === true) {
				...
				for (const item of autoSavingItems) items.push(item);
