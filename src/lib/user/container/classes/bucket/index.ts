...
export class Bucket<T extends Template> implements BucketImplementation<T> {
    ...
    private readonly load_jobs: Map<string, { id: number, data_saved: [DataSaved | Template, DataStoreKeyInfo] | undefined }> = new Map()
    ...
    public async getItem(itemKey: string): Promise<ItemType<T> | undefined> {
        assert(type(itemKey) === "string" || itemKey.size() !== 0, "[Container_Bucket[%s]]: Item's key can't be of another type than a string and it needs to have a length > 0.")
        ...
        while (servicesAvailable === true) {
            let loadedData!: DataSaved | Template; let keyInfo!: DataStoreKeyInfo;
            ...
            if (loadJob !== undefined) {
                ...
                loadJob.data_saved = dataStoreQueueCall(
                    itemKey,
                    this,
                    {
                        setItem: <T>(object: T, keyInfo: DataStoreKeyInfo) => { }
                    }
                )
            }
